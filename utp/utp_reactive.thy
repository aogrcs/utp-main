...
overloading
  ...
  rea_unit == "utp_unit :: (REA, ('t::ordered_cancel_monoid_diff,'\<alpha>) alphabet_rp) uthy \<Rightarrow> ('t,'\<alpha>) hrelation_rp"
begin
  definition rea_hcond :: "(REA, ('t::ordered_cancel_monoid_diff,'\<alpha>) alphabet_rp) uthy \<Rightarrow> (('t,'\<alpha>) alphabet_rp \<times> ('t,'\<alpha>) alphabet_rp) Healthiness_condition"
  where [upred_defs]: "rea_hcond T = RP"
  definition rea_unit :: "(REA, ('t::ordered_cancel_monoid_diff,'\<alpha>) alphabet_rp) uthy \<Rightarrow> ('t,'\<alpha>) hrelation_rp"
...
interpretation rea_utp_theory_rel: utp_theory_unital "UTHY(REA, ('t::ordered_cancel_monoid_diff,'\<alpha>) alphabet_rp)"
  rewrites "carrier (uthy_order REA) = \<lbrakk>RP\<rbrakk>\<^sub>H"
